...
const columns: { status: Status; label: string; color: string; dot: string }[] = [
  { status: "planned", label: "Planned", color: "text-neutral-400", dot: "bg-neutral-600" },
...
const productColors: Record<string, string> = {
  ...
  Platform: "text-neutral-400",
}

export default function RoadmapSection() {
  return (
    <section id="roadmap" className="py-24 px-4 sm:px-6 lg:px-8 bg-neutral-950">
      <div className="max-w-6xl mx-auto">
            ...
            <FadeIn key={col.status} delay={ci * 100}>
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="ml-auto text-xs text-neutral-600">{items[col.status].length}</span>
                </div>
                {items[col.status].map((item, i) => (
                  <div key={i} className="rounded-xl border border-neutral-800 bg-neutral-900 p-4 flex flex-col gap-1.5">
        ...
        <FadeIn delay={400}>
          <p className="mt-10 text-center text-xs text-neutral-700">
            ...
            <a href="mailto:hello@wkllabs.com" className="text-neutral-500 hover:text-white transition-colors underline underline-offset-2">
              Send us an email
            </a>
          </p>
        </FadeIn>
